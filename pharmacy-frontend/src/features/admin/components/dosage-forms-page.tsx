"use client";

import {
  Pencil,
  Plus,
  Trash2,
  X,
  Pill,
  FlaskConical,
} from "lucide-react";
import { useCallback, useEffect, useState } from "react";

import { PrimaryButton, SecondaryButton } from "@/components/ui/buttons";
import { ConfirmDialog } from "@/components/ui/confirm-dialog";
import { EmptyState } from "@/components/ui/empty-state";
import { Field, FormError, Input, Textarea } from "@/components/ui/form-controls";
import { PageHeader } from "@/components/ui/page-header";
import { AccessRestricted } from "@/features/auth/components/access-restricted";
import { PERMISSIONS } from "@/features/auth/access-control";
import { usePermission } from "@/features/auth/hooks/use-permission";
import { apiRequest, ApiClientError } from "@/lib/api-client";

interface DosageForm {
  id: string;
  name: string;
  abbreviation: string;
  description: string;
  createdAt: string;
  updatedAt: string;
}

function errorMessage(error: unknown, fallback: string) {
  return error instanceof ApiClientError || error instanceof Error
    ? error.message
    : fallback;
}

export function DosageFormsPage() {
  const canWrite = usePermission(PERMISSIONS.MEDICINE_WRITE);
  const canRead = usePermission(PERMISSIONS.MEDICINE_READ);
  const [forms, setForms] = useState<DosageForm[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [name, setName] = useState("");
  const [abbreviation, setAbbreviation] = useState("");
  const [description, setDescription] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [deleteTarget, setDeleteTarget] = useState<DosageForm | null>(null);
  const [deleting, setDeleting] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiRequest<DosageForm[]>("/dosage-forms");
      setForms(res.data);
      setError(null);
    } catch (caught) {
      setError(errorMessage(caught, "Dosage forms could not be loaded."));
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => {
    if (!canRead) return;
    let active = true;
    void apiRequest<DosageForm[]>("/dosage-forms")
      .then((res) => {
        if (active) {
          setForms(res.data);
          setError(null);
        }
      })
      .catch((caught) => {
        if (active) setError(errorMessage(caught, "Dosage forms could not be loaded."));
      })
      .finally(() => {
        if (active) setLoading(false);
      });
    return () => {
      active = false;
    };
  }, [canRead]);

  function resetForm() {
    setName("");
    setAbbreviation("");
    setDescription("");
    setEditingId(null);
    setError(null);
  }

  function openCreateForm() {
    resetForm();
    setShowForm(true);
  }

  function openEditForm(form: DosageForm) {
    setName(form.name);
    setAbbreviation(form.abbreviation);
    setDescription(form.description);
    setEditingId(form.id);
    setError(null);
    setShowForm(true);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  function closeForm() {
    setShowForm(false);
    resetForm();
  }

  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError(null);
    if (!canWrite) {
      setError("Your active roles do not permit dosage form changes.");
      return;
    }
    if (name.trim().length < 1) {
      setError("Enter a dosage form name.");
      return;
    }

    setSubmitting(true);
    try {
      const body = {
        name: name.trim(),
        abbreviation: abbreviation.trim(),
        description: description.trim(),
      };
      if (editingId) {
        await apiRequest(`/dosage-forms/${editingId}`, {
          method: "PUT",
          body,
        });
      } else {
        await apiRequest("/dosage-forms", {
          method: "POST",
          body,
        });
      }
      closeForm();
      await load();
    } catch (caught) {
      setError(
        errorMessage(
          caught,
          `The dosage form could not be ${editingId ? "updated" : "created"}.`,
        ),
      );
    } finally {
      setSubmitting(false);
    }
  }

  async function handleDelete() {
    if (!deleteTarget) return;
    setDeleting(true);
    try {
      await apiRequest(`/dosage-forms/${deleteTarget.id}`, { method: "DELETE" });
      setDeleteTarget(null);
      await load();
    } catch (caught) {
      setError(errorMessage(caught, "The dosage form could not be deleted."));
    } finally {
      setDeleting(false);
    }
  }

  if (!canRead) return <AccessRestricted />;

  return (
    <div>
      <PageHeader
        title="Dosage forms"
        description="Manage the list of dosage forms available for medicines in the pharmacy."
        actions={
          canWrite ? (
            <SecondaryButton
              type="button"
              onClick={showForm ? closeForm : openCreateForm}
            >
              {showForm ? (
                <X aria-hidden="true" size={17} />
              ) : (
                <Plus aria-hidden="true" size={17} />
              )}
              {showForm ? "Close" : "Add dosage form"}
            </SecondaryButton>
          ) : undefined
        }
      />

      {showForm ? (
        <form
          onSubmit={handleSubmit}
          className="mb-6 rounded-md border border-[var(--border)] bg-white p-4 sm:p-6"
        >
          <div className="flex items-center gap-2">
            <FlaskConical
              aria-hidden="true"
              className="text-[var(--brand)]"
              size={18}
            />
            <h2 className="text-base font-semibold">
              {editingId ? "Edit dosage form" : "New dosage form"}
            </h2>
          </div>

          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <Field label="Name" required>
              <Input
                autoFocus
                placeholder="e.g. Tablet"
                value={name}
                onChange={(event) => setName(event.target.value)}
              />
            </Field>
            <Field label="Abbreviation">
              <Input
                placeholder="e.g. TAB"
                value={abbreviation}
                onChange={(event) => setAbbreviation(event.target.value)}
              />
            </Field>
            <Field label="Description">
              <Input
                placeholder="Brief description"
                value={description}
                onChange={(event) => setDescription(event.target.value)}
              />
            </Field>
          </div>

          <div className="mt-4">
            <FormError message={error} />
          </div>
          <div className="mt-4 flex justify-end">
            <PrimaryButton type="submit" disabled={submitting}>
              {editingId ? (
                <Pencil aria-hidden="true" size={17} />
              ) : (
                <Plus aria-hidden="true" size={17} />
              )}
              {submitting
                ? "Saving..."
                : editingId
                  ? "Save changes"
                  : "Add dosage form"}
            </PrimaryButton>
          </div>
        </form>
      ) : null}

      {!showForm ? <FormError message={error} /> : null}

      <section
        className={`${!showForm && error ? "mt-4 " : ""}rounded-md border border-[var(--border)] bg-white`}
      >
        {forms.length ? (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[700px] text-left text-sm">
              <thead className="bg-[var(--surface-muted)] text-xs uppercase text-[var(--text-muted)]">
                <tr>
                  <th className="px-4 py-3 font-semibold">Name</th>
                  <th className="px-4 py-3 font-semibold">Abbreviation</th>
                  <th className="px-4 py-3 font-semibold">Description</th>
                  <th className="px-4 py-3 text-right font-semibold">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border)]">
                {forms.map((form) => (
                  <tr key={form.id} className="hover:bg-[var(--surface-muted)]/60">
                    <td className="px-4 py-3.5 font-semibold">{form.name}</td>
                    <td className="px-4 py-3.5 font-mono text-xs text-[var(--text-muted)]">
                      {form.abbreviation || "-"}
                    </td>
                    <td className="px-4 py-3.5 text-[var(--text-muted)]">
                      {form.description || "-"}
                    </td>
                    <td className="px-4 py-3.5">
                      {canWrite ? (
                        <div className="flex justify-end gap-1">
                          <button
                            type="button"
                            title={`Edit ${form.name}`}
                            aria-label={`Edit ${form.name}`}
                            onClick={() => openEditForm(form)}
                            className="flex size-9 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--surface-muted)] hover:text-[var(--text)]"
                          >
                            <Pencil aria-hidden="true" size={16} />
                          </button>
                          <button
                            type="button"
                            title={`Delete ${form.name}`}
                            aria-label={`Delete ${form.name}`}
                            onClick={() => setDeleteTarget(form)}
                            className="flex size-9 items-center justify-center rounded-md text-[var(--text-muted)] hover:bg-[var(--danger-soft)] hover:text-[var(--danger)]"
                          >
                            <Trash2 aria-hidden="true" size={16} />
                          </button>
                        </div>
                      ) : null}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        ) : loading ? (
          <div className="p-8 text-center text-sm text-[var(--text-muted)]">
            Loading dosage forms...
          </div>
        ) : (
          <EmptyState
            icon={Pill}
            title="No dosage forms"
            description="Dosage forms will appear here once created."
          />
        )}
      </section>

      <ConfirmDialog
        busy={deleting}
        busyLabel="Deleting..."
        confirmLabel="Delete dosage form"
        description={`This will permanently remove "${deleteTarget?.name}". This action cannot be undone.`}
        onCancel={() => setDeleteTarget(null)}
        onConfirm={() => void handleDelete()}
        open={Boolean(deleteTarget)}
        title="Delete dosage form"
      />
    </div>
  );
}
