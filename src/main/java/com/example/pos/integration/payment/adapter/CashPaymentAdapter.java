...
public class CashPaymentAdapter implements PaymentAdapter {
    ...
    public CashPaymentAdapter(PaymentGatewayFactory factory) {
    ...
    public PaymentResponse process(PaymentRequest request) {
        var gw = factory.getGateway("CASH");
        var r = gw.process(toGwRequest(request));
        return mapResponse(r);
    }

    @Override
    public PaymentResponse queryStatus(String ref) {
        var r = factory.getGateway("CASH").queryStatus(ref);
        return mapResponse(r);
    }

    @Override
    public PaymentResponse refund(String ref, BigDecimal amount) {
        var r = factory.getGateway("CASH").refund(ref, amount);
        return mapResponse(r);
    }
